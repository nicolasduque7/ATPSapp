...
export async function createStudent(input: StudentInput): Promise<ActionResult<Student>> {
  try {
    ...
    return { ok: true, data: mapStudentRow(data) };
  } catch (e) {
    unstable_rethrow(e);
    return { ok: false, error: e instanceof Error ? e.message : "Couldn't create student. Try again." };
  }
}

export async function updateStudent(id: string, input: StudentInput): Promise<ActionResult<Student>> {
  try {
    ...
    unstable_rethrow(e);
    return { ok: false, error: e instanceof Error ? e.message : "Couldn't save student. Try again." };
  }
}
